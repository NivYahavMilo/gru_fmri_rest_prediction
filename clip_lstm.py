...
def train_clip_predict_rest(df_clip, df_rest, args):
    # get X-y from df
    subject_list = df_clip['Subject'].unique()
    train_list = subject_list[:args["train_size"]]
    test_list = subject_list[args["train_size"]:]

    features = [ii for ii in df_clip.columns if 'feat' in ii]
    k_feat = len(features)
    print('number of classes = %d' % (args["k_class"]))

    # length of each clip
    clip_time = np.zeros(args["k_class"])
    rest_time = np.zeros(args["k_class"])
    for ii in range(args["k_class"]):
        class_df = df_clip[df_clip['y'] == ii]
        clip_time[ii] = np.max(np.unique(class_df['timepoint'])) + 1

        class_df_rest = df_rest[df_rest['y'] == ii]
        rest_time[ii] = np.max(np.unique(class_df_rest['timepoint'])) + 1
    clip_time = clip_time.astype(int)  # df saves float
    rest_time = rest_time.astype(int)  # df saves float
    ...
    init model
    '''
    model = LSTMClassifier(k_feat, args["k_hidden"], args["k_layers"], args["k_class"], bi_lstm=args["bi_lstm"])
    ...
    X_train, train_len, y_train = _get_seq(df_clip, train_list, args)
    X_test, test_len, y_test = _get_seq(df_rest, test_list, args)
    ...
    evaluate
    '''
    _info('test seq lengths = %s' % rest_time)


    # results dict init
    results = {}
    test_max_length = torch.max(test_len)
    a, a_t, c_mtx = _lstm_test_acc(model, X_test, y_test, test_len, test_max_length, rest_time, len(test_list))
    results['test'] = a
